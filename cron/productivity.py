...
class Outlet:
    ...
    def productivityReport(self,req):    
        try:
            ...
            id = ObjectId()
            name = 'Productivity-'+dateStr+"-"+str(id)+'.xlsx'
            ...
            worksheet1 = workbook.add_worksheet("Outlet Wise")
            worksheet1.set_column(0, 49, 20)
            c = 0        
            c = self.__insertWorksheet(c,"Outlet Wise",workbook,worksheet1,self.outlets)
            ...
            if result:   
                for row in result:
                    ...
                    try:
                        ...
                        t['OUTLET AREA'] = row['outlet']['area']
                        t['OUTLET ZONE'] = row['outlet']['zone']
                        t['CITY'] = city[str(row['outlet']['city_id'])]
