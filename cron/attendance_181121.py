...
class Attendance:
    ...
    def getDays(self,date1):
        ...
        for n in range(int ((endDate - startDate).days)+1):
            ...
            self.row[d.strftime("%d-%b")+"-Timestamp"] = '-'
            self.row[d.strftime("%d-%b")+"-Check-In"] = '-'
            self.row[d.strftime("%d-%b")+"-Check-Out"] = '-'
        ...
        try:
            for row in data:
                ...
                if start == '':
                    ...
                    check_in = "Lat: "+str(c['latitude']) + ' Long: ' + str(c['latitude'])
                if end1['date'] != None:
                    end   = str(end1['date'])
                    c =  end1['coardinates']
                    check_out = "Lat: "+str(c['latitude']) + ' Long: ' + str(c['latitude'])
            
            timestamp =  start+' / '+end 
            return (timestamp,check_in,check_out)
            #return timestamp
        except Exception as e:
    ...
    def getRoutePlan(self):
        try:
            ...
            for row in result:
                d = row['date']
                timestamp,check_in,check_out = self.getActivity(row['checkin_activity'])
                #print(timestamp,check_in,check_out)
                self.routePlans[d.strftime("%d-%b")][str(row['fwp_id'])] = (timestamp,check_in,check_out)
    ...
    def attendanceReport(self,req):
        ...
        try:
            ...
            for row in result:
                ...
                try:
                    timestamp,check_in,check_out = self.routePlans[row['date'].strftime("%d-%b")][str(row['user_id'])]
