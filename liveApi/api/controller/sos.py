...
from api.lib.helper import getData,getId,getCurrentUserId, dateTimeValidate,check_permissions
    ...
    def post(self):
        try:
            ...
            if v.validate(json):
                ...
                #Update for locked device
                mongo.db.users.update_one({"_id":getId(getCurrentUserId())},{"$set": {'isLocked':1}})
...
class SOSNotificationById(Resource):
    ...
    def put(self,id):
        try:
            id = request.args.get('sosId', None)
            mongo.db.sos_notification.update_one({'_id':getId(id)},{"$set": {'read':1}})
            result = mongo.db.sos_notification.find_one({'_id':getId(id)})
            if result:
                user_id = result['user_id']
                #Update for locked device
                #mongo.db.users.update_one({"_id":user_id},{"$set": {'isLocked':0}})
