...
from api.lib.helper import getData,getId,getRow,getCurrentUserId,dateTimeValidate,check_permissions
    ...
    def put(self,id):
        try:
            ...
            if v.validate(json):
                ...
                result = mongo.db.form_fields.find_one_or_404({"_id":getId(id)})
                ## commented by sagar(client) to update all existing form -- 27/01/2022
                # ~ if result['fixed'] == 1:
...
# as per sagar we have added the min age validation 18 before it validation key in db table is 
#"validation":{"empty":false,"min":0,"required":true,"type":"string"}
