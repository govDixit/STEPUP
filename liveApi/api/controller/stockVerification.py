...
from api.lib.helper import getData,getId,getCurrentUserId,check_permissions
...
class OutletVerification(Resource):
    ...
    def post(self,user_id):
        try:
            ...
            if v.validate(json):
                ...
                newvalues = {"$set": {"list":[],"step"+str(json['step'])+"_verify" :1,"step"+str(json['step'])+"_data" : self.verify_data}}
                ...
                self.updateFormComment(json['list'],json['verify'], json['comments'])
    ...
    def updateFormComment(self,list,status,comments):
        try:
            for id in list:
                myquery = {"_id":getId(id)}
                newvalues = {"$set": {'status':int(status),'comments':comments}}
                
                r = mongo.db.miform.update_one(myquery, newvalues)
