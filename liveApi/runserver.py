...
# ~ or request.endpoint == "reportdownload"
@app.before_request
def before_request(): 
    #print(request.headers)
    if not (request.endpoint == 'login' or request.endpoint == 'refresh' or request.endpoint == 'isLocked' or request.endpoint == 'assets' or request.endpoint == "learningdownload" or request.endpoint == "skillsdownload" or request.endpoint == "signaturedownload" or request.endpoint == "checkindownload" or request.endpoint == "userpic"): 
        #jwt_required() 
        try:
            verify_jwt_in_request()
        except:
            return jsonify({"msg":"Your token has expired"}),401
